...
import com.tamil.thamizhpesi.audio.processor.SoundFormat.Encoding;
 ...
 */
public abstract class SoundFloatConverter {
    ...
    private static class SoundFloatLSBFilter extends SoundFloatConverter {
        ...
        public SoundFloatLSBFilter(SoundFloatConverter converter,
                                   SoundFormat format) {
    ...
    private static class SoundFloatConversion64L extends SoundFloatConverter {
    ...
    private static class SoundFloatConversion64B extends SoundFloatConverter {
        ByteBuffer bytebuffer = null;
    ...
    private static class SoundFloatConversion32L extends SoundFloatConverter {
        ByteBuffer bytebuffer = null;
    ...
    private static class SoundFloatConversion32B extends SoundFloatConverter {
        ByteBuffer bytebuffer = null;
    ...
    // PCM 8 bit, signed
    private static class SoundFloatConversion8S extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    // PCM 8 bit, unsigned
    private static class SoundFloatConversion8U extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion16SL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
    ...
    private static class SoundFloatConversion16SB extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion16UL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion16UB extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion24SL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion24SB extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion24UL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion24UB extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion32SL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion32SB extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion32UL extends SoundFloatConverter {
        public float[] toFloatArray(byte[] in_buff, int in_offset,
                                    float[] out_buff, int out_offset, int out_len) {
            int ix = in_offset;
            int ox = out_offset;
    ...
    private static class SoundFloatConversion32UB extends SoundFloatConverter {
    ...
    // PCM 32+ bit, signed, little-endian
    private static class SoundFloatConversion32XSL extends SoundFloatConverter {

        final int xbytes;

        public SoundFloatConversion32XSL(int xbytes) {
            this.xbytes = xbytes;
        }
    ...
    // PCM 32+ bit, signed, big-endian
    private static class SoundFloatConversion32XSB extends SoundFloatConverter {

        final int xbytes;

        public SoundFloatConversion32XSB(int xbytes) {
            this.xbytes = xbytes;
        }
    ...
    // PCM 32+ bit, unsigned, little-endian
    private static class SoundFloatConversion32XUL extends SoundFloatConverter {

        final int xbytes;

        public SoundFloatConversion32XUL(int xbytes) {
            this.xbytes = xbytes;
        }
    ...
    // PCM 32+ bit, unsigned, big-endian
    private static class SoundFloatConversion32XUB extends SoundFloatConverter {

        final int xbytes;

        public SoundFloatConversion32XUB(int xbytes) {
            this.xbytes = xbytes;
        }
    ...
    public static SoundFloatConverter getConverter(SoundFormat format) {
        SoundFloatConverter conv = null;
        ...
        if (format.getEncoding().equals(Encoding.PCM_SIGNED)) {
            if (format.isBigEndian()) {
                if (format.getSampleSizeInBits() <= 8) {
                    conv = new SoundFloatConversion8S();
                } else if (format.getSampleSizeInBits() > 8 &&
                        format.getSampleSizeInBits() <= 16) {
                    conv = new SoundFloatConversion16SB();
                } else if (format.getSampleSizeInBits() > 16 &&
                        format.getSampleSizeInBits() <= 24) {
                    conv = new SoundFloatConversion24SB();
                } else if (format.getSampleSizeInBits() > 24 &&
                        format.getSampleSizeInBits() <= 32) {
                    conv = new SoundFloatConversion32SB();
                } else if (format.getSampleSizeInBits() > 32) {
                    conv = new SoundFloatConversion32XSB(((format
                ...
                if (format.getSampleSizeInBits() <= 8) {
                    conv = new SoundFloatConversion8S();
                        ...
                        format.getSampleSizeInBits() <= 16) {
                    conv = new SoundFloatConversion16SL();
                } else if (format.getSampleSizeInBits() > 16 &&
                        format.getSampleSizeInBits() <= 24) {
                    conv = new SoundFloatConversion24SL();
                } else if (format.getSampleSizeInBits() > 24 &&
                        format.getSampleSizeInBits() <= 32) {
                    conv = new SoundFloatConversion32SL();
                } else if (format.getSampleSizeInBits() > 32) {
                    conv = new SoundFloatConversion32XSL(((format
                            .getSampleSizeInBits() + 7) / 8) - 4);
                }
            }
            ...
            if (format.isBigEndian()) {
                if (format.getSampleSizeInBits() <= 8) {
                    conv = new SoundFloatConversion8U();
                } else if (format.getSampleSizeInBits() > 8 &&
                        format.getSampleSizeInBits() <= 16) {
                    conv = new SoundFloatConversion16UB();
                } else if (format.getSampleSizeInBits() > 16 &&
                        format.getSampleSizeInBits() <= 24) {
                    conv = new SoundFloatConversion24UB();
                } else if (format.getSampleSizeInBits() > 24 &&
                        format.getSampleSizeInBits() <= 32) {
                    conv = new SoundFloatConversion32UB();
                } else if (format.getSampleSizeInBits() > 32) {
                    conv = new SoundFloatConversion32XUB(((
                ...
                if (format.getSampleSizeInBits() <= 8) {
                    conv = new SoundFloatConversion8U();
                        ...
                        format.getSampleSizeInBits() <= 16) {
                    conv = new SoundFloatConversion16UL();
                } else if (format.getSampleSizeInBits() > 16 &&
                        format.getSampleSizeInBits() <= 24) {
                    conv = new SoundFloatConversion24UL();
                } else if (format.getSampleSizeInBits() > 24 &&
                        format.getSampleSizeInBits() <= 32) {
                    conv = new SoundFloatConversion32UL();
                } else if (format.getSampleSizeInBits() > 32) {
                    conv = new SoundFloatConversion32XUL(((
                            format.getSampleSizeInBits() + 7) / 8) - 4);
                }
            }
            ...
            if (format.getSampleSizeInBits() == 32) {
                if (format.isBigEndian())
                    conv = new SoundFloatConversion32B();
                else
                    conv = new SoundFloatConversion32L();
            } else if (format.getSampleSizeInBits() == 64) {
                if (format.isBigEndian())
                    conv = new SoundFloatConversion64B();
                else
                    conv = new SoundFloatConversion64L();
            }
        ...
        if ((format.getEncoding().equals(Encoding.PCM_SIGNED) ||
            ...
            conv = new SoundFloatLSBFilter(conv, format);
        }
