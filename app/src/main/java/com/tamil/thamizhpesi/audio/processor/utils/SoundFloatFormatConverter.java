 ...
 */
public class SoundFloatFormatConverter extends FormatConversionProvider {

    private static class AudioFloatFormatConverterInputStream extends
            InputStream {
        private SoundFloatConverter converter;

        private SoundFloatInputStream stream;

        private float[] readfloatbuffer;

        private int fsize = 0;

        public AudioFloatFormatConverterInputStream(SoundFormat targetFormat,
                                                    SoundFloatInputStream stream) {
            this.stream = stream;
            converter = SoundFloatConverter.getConverter(targetFormat);
    ...
    private static class SoundFloatInputStreamChannelMixer extends
            SoundFloatInputStream {

        private int targetChannels;

        private int sourceChannels;

        private SoundFloatInputStream ais;

        private SoundFormat targetFormat;

        private float[] conversion_buffer;

        public SoundFloatInputStreamChannelMixer(SoundFloatInputStream ais,
                                                 int targetChannels) {
            this.sourceChannels = ais.getFormat().getChannels();
            this.targetChannels = targetChannels;
            this.ais = ais;
            SoundFormat format = ais.getFormat();
            targetFormat = new SoundFormat(format.getEncoding(), format
    ...
    private static class SoundFloatInputStreamResampler extends
            SoundFloatInputStream {
  ...
  /*      public SoundFloatInputStreamResampler(SoundFloatInputStream ais,
                                              SoundFormat format) {
            this.ais = ais;
            SoundFormat sourceFormat = ais.getFormat();
            targetFormat = new SoundFormat(sourceFormat.getEncoding(), format
        ...
        public long getFrameLength() {
            return SoundSystem.NOT_SPECIFIED; // ais.getFrameLength();
        }
    ...
    public SoundInputStream getAudioInputStream(Encoding targetEncoding,
                                                SoundInputStream sourceStream) {
        if (sourceStream.getFormat().getEncoding().equals(targetEncoding))
            return sourceStream;
        SoundFormat format = sourceStream.getFormat();
        int channels = format.getChannels();
        Encoding encoding = targetEncoding;
        float samplerate = format.getSampleRate();
        int bits = format.getSampleSizeInBits();
        boolean bigendian = format.isBigEndian();
        if (targetEncoding.equals(SoundFloatConverter.PCM_FLOAT))
            bits = 32;
        SoundFormat targetFormat = new SoundFormat(encoding, samplerate, bits,
                channels, channels * bits / 8, samplerate, bigendian);
        return getAudioInputStream(targetFormat, sourceStream);
    }

    public SoundInputStream getAudioInputStream(SoundFormat targetFormat,
                                                SoundInputStream sourceStream) {
        if (!isConversionSupported(targetFormat, sourceStream.getFormat()))
            throw new IllegalArgumentException("Unsupported conversion: "
                    + sourceStream.getFormat().toString() + " to "
                    + targetFormat.toString());
        return getAudioInputStream(targetFormat, SoundFloatInputStream
                .getInputStream(sourceStream));
    }

    public SoundInputStream getAudioInputStream(SoundFormat targetFormat,
                                                SoundFloatInputStream sourceStream) {
        ...
        if (targetFormat.getChannels() != sourceStream.getFormat()
                .getChannels())
            sourceStream = new SoundFloatInputStreamChannelMixer(sourceStream,
                    targetFormat.getChannels());
       /* if (Math.abs(targetFormat.getSampleRate()
                - sourceStream.getFormat().getSampleRate()) > 0.000001)
            sourceStream = new SoundFloatInputStreamResampler(sourceStream,
                    targetFormat);*/
        return new SoundInputStream(new AudioFloatFormatConverterInputStream(
    ...
    public Encoding[] getTargetEncodings(SoundFormat sourceFormat) {
        if (SoundFloatConverter.getConverter(sourceFormat) == null)
            return new Encoding[0];
        return new Encoding[] { Encoding.PCM_SIGNED, Encoding.PCM_UNSIGNED,
                SoundFloatConverter.PCM_FLOAT };
    }

    public SoundFormat[] getTargetFormats(Encoding targetEncoding,
                                          SoundFormat sourceFormat) {
        if (SoundFloatConverter.getConverter(sourceFormat) == null)
            return new SoundFormat[0];
        int channels = sourceFormat.getChannels();

        ArrayList<SoundFormat> formats = new ArrayList<SoundFormat>();

        if (targetEncoding.equals(Encoding.PCM_SIGNED))
            formats.add(new SoundFormat(Encoding.PCM_SIGNED,
                    SoundSystem.NOT_SPECIFIED, 8, channels, channels,
                    SoundSystem.NOT_SPECIFIED, false));
        if (targetEncoding.equals(Encoding.PCM_UNSIGNED))
            formats.add(new SoundFormat(Encoding.PCM_UNSIGNED,
                    SoundSystem.NOT_SPECIFIED, 8, channels, channels,
                    SoundSystem.NOT_SPECIFIED, false));
        ...
        for (int bits = 16; bits < 32; bits += 8) {
            if (targetEncoding.equals(Encoding.PCM_SIGNED)) {
                formats.add(new SoundFormat(Encoding.PCM_SIGNED,
                        SoundSystem.NOT_SPECIFIED, bits, channels, channels
                        * bits / 8, SoundSystem.NOT_SPECIFIED, false));
                formats.add(new SoundFormat(Encoding.PCM_SIGNED,
                        SoundSystem.NOT_SPECIFIED, bits, channels, channels
                        * bits / 8, SoundSystem.NOT_SPECIFIED, true));
            }
            if (targetEncoding.equals(Encoding.PCM_UNSIGNED)) {
                formats.add(new SoundFormat(Encoding.PCM_UNSIGNED,
                        SoundSystem.NOT_SPECIFIED, bits, channels, channels
        ...
        if (targetEncoding.equals(SoundFloatConverter.PCM_FLOAT)) {
            formats.add(new SoundFormat(SoundFloatConverter.PCM_FLOAT,
                    SoundSystem.NOT_SPECIFIED, 32, channels, channels * 4,
                    SoundSystem.NOT_SPECIFIED, false));
            formats.add(new SoundFormat(SoundFloatConverter.PCM_FLOAT,
                    SoundSystem.NOT_SPECIFIED, 32, channels, channels * 4,
                    SoundSystem.NOT_SPECIFIED, true));
            formats.add(new SoundFormat(SoundFloatConverter.PCM_FLOAT,
                    SoundSystem.NOT_SPECIFIED, 64, channels, channels * 8,
                    SoundSystem.NOT_SPECIFIED, false));
            formats.add(new SoundFormat(SoundFloatConverter.PCM_FLOAT,
                    SoundSystem.NOT_SPECIFIED, 64, channels, channels * 8,
                    SoundSystem.NOT_SPECIFIED, true));
        ...
        return formats.toArray(new SoundFormat[formats.size()]);
    }

    public boolean isConversionSupported(SoundFormat targetFormat,
                                         SoundFormat sourceFormat) {
        if (SoundFloatConverter.getConverter(sourceFormat) == null)
            return false;
        if (SoundFloatConverter.getConverter(targetFormat) == null)
            return false;
