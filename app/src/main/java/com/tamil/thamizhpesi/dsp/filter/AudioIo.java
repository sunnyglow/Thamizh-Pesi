...
private static class AudioBytesPackerStream extends InputStream {
   ...
   int             inLen;
   int             pos;
   public AudioBytesPackerStream (SoundFormat format, float[][] inBufs, int inOffs, int inLen) {
      this.format = format;
      this.inBufs = inBufs;
      this.inOffs = inOffs;
      this.inLen  = inLen; }
...
public static AudioSignal loadWavFile (String fileName) throws Exception {
   ...
   try {
      AudioSignal signal = new AudioSignal();
      stream = SoundSystem.getAudioInputStream(new File(fileName));
...
*/
public static void unpackAudioStreamBytes (SoundFormat format, byte[] inBuf, int inPos, float[][] outBufs, int outPos, int frames) {
...
private static void unpackAudioStreamBytesPcmSigned (SoundFormat format, byte[] inBuf, int inPos, float[][] outBufs, int outPos, int frames) {
   int channels = format.getChannels();
...
private static void unpackAudioStreamBytesPcmFloat (SoundFormat format, byte[] inBuf, int inPos, float[][] outBufs, int outPos, int frames) {
   int channels = format.getChannels();
   boolean bigEndian = format.isBigEndian();
   int sampleBits = format.getSampleSizeInBits();
   int frameSize = format.getFrameSize();
...
*/
public static void packAudioStreamBytes (SoundFormat format, float[][] inBufs, int inPos, byte[] outBuf, int outPos, int frames) {
   Encoding encoding = format.getEncoding();
   if (encoding == Encoding.PCM_SIGNED) {
...
private static void packAudioStreamBytesPcmSigned (SoundFormat format, float[][] inBufs, int inPos, byte[] outBuf, int outPos, int frames) {
   int channels = format.getChannels();
   boolean bigEndian = format.isBigEndian();
   int sampleBits = format.getSampleSizeInBits();
   int frameSize = format.getFrameSize();
...
private static void packAudioStreamBytesPcmFloat (SoundFormat format, float[][] inBufs, int inPos, byte[] outBuf, int outPos, int frames) {
   int channels = format.getChannels();
   boolean bigEndian = format.isBigEndian();
   int sampleBits = format.getSampleSizeInBits();
   int frameSize = format.getFrameSize();
